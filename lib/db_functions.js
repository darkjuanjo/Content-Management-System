    ...
    return db.getRoles().then(rows => {
        ...
        rows[0].forEach(element => {
            ...
            return inquirer
                ...
                .then(selected_role => {
                change.role = selected_role.role;
                return db.getDepartments()
                .then(rows => {
                    rows[0].forEach(element => {
                        const department_object = {
                    ...
                    .prompt([
                        {
                            type: 'list',
                            name: 'department',
                            message: 'Please select new department',
                            choices: departments
                        }
                    ])
                    .then(selected_department => {
                        change.department = selected_department.department;
                        console.log(change);
                    return db.updateRoleDepartment(change)
                    .then(() => db.getRoles()
                    .then(rows => {
                        return rows[0];
                    }))
